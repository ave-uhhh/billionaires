...
document.addEventListener("DOMContentLoaded", function () {
  if (bezos_counter) {
      bezos_counter.innerHTML = "$0"; // Ensures it starts at zero
  }
});
...
function update_wealth_counter() {
  if (bezos_viewable()) {
      if (bezos_counter_viewable()) {
          let scrollDistance = Math.max(0, window.scrollX - bezos.offsetLeft); 
          if (scrollDistance < 0) scrollDistance = 0;
          let wealth = scrollDistance * 500000; // Calculate wealth dynamically
          bezos_counter.innerHTML = money.format(wealth);
      } else {
      ...
      if (four_hundred_counter_viewable()) {
          let scrollDistance = Math.max(0, window.scrollX - four_hundred.offsetLeft);
          let wealth = scrollDistance * 500000;
          four_hundred_counter.innerHTML = money.format(wealth);
      } else {
